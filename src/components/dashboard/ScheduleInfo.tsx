...
export default function ScheduleInfo({ selectedDate }: { selectedDate: string }) {
        ...
        <div className="flex flex-col items-center justify-center pt-6 text-xl gap-y-5">
            {matchingTours.length > 0 ? (
                matchingTours.map((tour) => (
                    <div key={tour.date} className="grid lg:grid-cols-3 grid-cols-4 items-center rounded-xl w-full py-7 bg-[#0F5E5A] text-white gap-x-5">
                ...
                <Link href='/calendar' className="flex items-center justify-center bg-[#0F5E5A] rounded-xl py-4 w-full md:w-10/12">
